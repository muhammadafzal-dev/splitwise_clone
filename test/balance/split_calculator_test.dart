...
void main() {
  ...
  group('SplitCalculator EQUAL', () {
    ...
    test('should_throw_when_no_participants', () {
      expect(
        () => calc.computeShares(equalExpense(
          payer: 'a',
          amount: 100,
          participants: [],
        )),
  ...
  group('SplitCalculator EXACT', () {
    test('should_return_provided_shares_when_they_sum_to_total', () {
      final shares = calc.computeShares(exactExpense(
        payer: 'a',
        amount: 1000,
        shares: {'a': 600, 'b': 400},
      ));
    ...
    test('should_throw_when_shares_over_total', () {
      expect(
        () => calc.computeShares(exactExpense(
          payer: 'a',
          amount: 1000,
          shares: {'a': 600, 'b': 500},
        )),
  ...
  group('SplitCalculator PERCENT', () {
    test('should_apply_largest_remainder_rounding', () {
      final shares = calc.computeShares(percentExpense(
        payer: 'a',
        amount: 1000,
    ...
    test('should_throw_when_percentages_do_not_sum_to_100', () {
      expect(
        () => calc.computeShares(percentExpense(
          payer: 'a',
          amount: 1000,
